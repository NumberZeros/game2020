...
void CSimon::UpdateIntro(int t, int &delay)
{
	if (x >= G_ScreenWidth / 2)
	{
		GSObject->Update(t);
		x += (Vx - 2.5);
		if (GSObject->_index > 3) { GSObject->_index = 0; }
	}
	else
	{
		GSObject->SelectIndex(9);
		delay --;// muốn đứng lại chỗ cổng thành bao lâu
	}
