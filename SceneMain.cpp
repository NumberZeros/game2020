...
void SceneMain::ProcessInput(LPDIRECT3DDEVICE9 d3ddv, int Delta)
{
	...
	if (IsKeyDown(DIK_4)) { State = 2; LoadMap2(); LoadMap21(); score->SetHeart(100); }
	if (IsKeyDown(DIK_5)) { State = 2; LoadMap2(); LoadMap22(); score->SetHeart(100); }
	if (IsKeyDown(DIK_6)) { State = 3; LoadMap3(); LoadMap31(); score->SetHeart(100);}
	if (IsKeyDown(DIK_7)) { State = 3; LoadMap3(); LoadMap32(); score->SetHeart(100); }
	if (IsKeyDown(DIK_8)) { State = 3; LoadMap3(); LoadMap33(); score->SetHeart(100); }
	if (IsKeyDown(DIK_9)) { State = 3; LoadMap3(); LoadMap34(); score->SetHeart(100); }
	if (IsKeyDown(DIK_0)) { State = 3; LoadMap3(); LoadMap35(); score->SetHeart(100); }
	if (IsKeyDown(DIK_3)) { State = 3; LoadMap3(); }
	...
	if (IsKeyDown(DIK_W)) {
		weapons.push_back(new CBottle());
	}
	if (IsKeyDown(DIK_E)) {
		weapons.push_back(new CBumeran());
	}
	if (IsKeyDown(DIK_R)) {
		weapons.push_back(new CAxe());
	}*/
		...
		if (simon->GetOnStair()) {
			...
			if (IsKeyDown(DIK_T)) {
				if (simon->getAttacking() == 0) {
					simon->Attack(weapons.at(0)); sound->PlaySoundChoose(18);
				}
			}
		}
		else {
			...
			if (IsKeyDown(DIK_Y)) {
				if (simon->getAttacking() == 0) {
					...
					if (typeWeapon != 0 && score->GetHeart() > 0) {
						score->_Heart--; sound->PlaySoundChoose(9);
					}
				}
			}
...
void SceneMain::LoadMap22()
{
	...
	cam = new GCamera(simon->getx(), 2);
	camObj = new GCamera(simon->getx(), 1);
	cam->SetSizeMap(4096, G_MapWidth);
	camObj->SetSizeMap(4096, G_MapWidth);
	Floor = 2;
}
void SceneMain::LoadMap3()
{
	...
	simon = new CSimon(G_ScreenWidth, 76, 1);
	Enemys->clear();
	//Load lại xét trạng thái tấn công của boss
	cohieu = true;//Xét trạng thái tấn công của boss bat
	MedusaAction_Time = 0;//Xét trạng thái tấn công boss medusa
	//QT3
	QT = new QuadTree(State);
	simon->setx(3650);
	simon->sety(64);
	cam = new GCamera(simon->getx(), 3);
	cam->viewport.y = 448;
	cam->SetSizeMap(3584, 4096);
	camObj = new GCamera(simon->getx(), 1);
	camObj->SetSizeMap(3584, 4096);
	score->SetTime(400);
	score->SetHeart(5);
	score->SetEnemyBlood(16);
	score->SetSimonBlood(16);
	Floor = 1;
	//Khởi tạo đi tới QT3b
	//LoadMap31();
	//Khởi tạo tạm để đi tới QT3c
	//LoadMap32();
	//Khởi tạo tạm để đi tới QT3d
	//LoadMap33();
	//QT3e
	//LoadMap34();
	//QT3f
...
void SceneMain::LoadMap31()
{
	...
	cam = new GCamera(simon->getx(), 3);
	cam->viewport.y = 832;
	cam->SetSizeMap(3072, 4096);
	camObj = new GCamera(simon->getx(), 1);
	camObj->SetSizeMap(3072, 4096);
	Floor = 2;
}
//Load QT3c
void SceneMain::LoadMap32()
{
	...
	cam = new GCamera(simon->getx(), 3);
	cam->viewport.y = 832;
	cam->SetSizeMap(1536, 3072);
	camObj = new GCamera(simon->getx(), 1);
	camObj->SetSizeMap(1536, 3072);
	Floor = 2;
}
//Load QT3d
...
void SceneMain::RenderMenu(int t)
{
	if (isPlay_Exit)
	{
		play->Update(t); exit->Set();
	}
	else
	{
		exit->Update(t); play->Set();
	}
	play->Draw();
	exit->Draw();
}
...
void SceneMain::RenderIntro(int t)
{
	...
	if (delay > 0)
	{
		simon->UpdateIntro(t, delay);
		simon->Draw();
	}
	else// bắt đầu vào màn 1 khi delay kết thúc
	{
		simon->setIndexSimon();
		//lockKeyboard = 0;
		State = 1;
...
void SceneMain::RenderStage(LPDIRECT3DDEVICE9 d3ddv, int t)
{
	...
	for (map<int, CDistinct*>::iterator obj = Enemys->begin(); obj != Enemys->end(); obj++) {
		...
		//khởi động con cá
		if (obj->second->getwhichObj() == 8 && obj->second->coliFish == false){
			if (obj->second->getIsFalling() == 0){
				obj->second->Jump();
				sound->PlaySoundChoose(16);
			}
			if (obj->second->getWater() == 0){
				obj->second->setWater(50);
				Effects.push_back(new CWater(obj->second->getObjX(), obj->second->getObjY()));
			}
		}
		//Nếu con cá hoặc con dragonhead tấn công
		if ((obj->second->getwhichObj() == 8 && obj->second->coliFish == true) || obj->second->getwhichObj() == 14){
			obj->second->Go();
			//Vẽ weaponEnemy
			//if (obj->second->coliBrick){
			if (((((obj->first == 44 || obj->first == 45) && obj->second->gettrend() == 1) || obj->first != 44 && obj->first != 45) && obj->second->getwhichObj() == 8) || obj->second->getwhichObj() == 14){
				if (obj->second->CreateWeapon()){
					weaponsEnemy.push_back(new CFireBall());
					obj->second->Attack(weaponsEnemy.back());
				}
			}
		}
		//Khởi tạo con boss medusa
		if (obj->second->getwhichObj() == 15){
			obj->second->GetSMPositionForBoss(simon->getx());
			if (MedusaAction_Time != 0)
			{
				MedusaAction_Time++;
				if (MedusaAction_Time == 50)
				{
					sound->StopSound();
					soundNow = 11;
					obj->second->Go();
			...
			if (abs(obj->second->getObjX() - simon->getx()) <= 20){			
					//Cố định cam
				if (MedusaAction_Time == 0)
				{
					cam->viewport.x = 0;
					cam->SetSizeMap(0, G_ScreenWidth);
					MedusaAction_Time++;
				}							
			}
			if (obj->second->CreateWeapon()) // Nếu là Boss Medusa ở trạng thái tấn công
			{
				weaponsEnemy.push_back(new SnakeWeapon());
				obj->second->Attack(weaponsEnemy.back());
			}
		}
		//Khoi tao con boss bat
		if (obj->second->getwhichObj() == 11) {
			obj->second->GetSMPositionForBoss(simon->getx(), simon->gety(), simon->getTrend());
			if (obj->second->getObjX() < simon->getx() - 20) {
				if (cohieu == true) {
					cohieu = false;
					obj->second->Go();
					this->SetDieAllEnemy();
					sound->StopSound();
					soundNow = 11;
					cam->viewport.x = G_MapWidth - G_ScreenWidth;
	...
	//if (State == 2) simon->Draw(cam); //^^^19
	board->Draw();
	boardItem->Draw();
	blood->DrawBlood();
	blood->DrawBloodSimon(score->GetSimonBlood());
	blood->DrawBloodEnemy(score->GetEnemyBlood());
	FlashAnimation();
	G_SpriteHandler->End();
	text->DrawTextTop(d3ddv, score->GetScore(), score->GetTime(), State, score->GetHeart(), score->GetPower());
...
void SceneMain::UpdateObject(GCamera * cam, int t){
	if (EndingScene == 1 && EndScene1() && simon->getx() >= 1450){
		...
		if (score->GetTime() == 0 && score->GetHeart() == 0 && State == 2) {
			State = 3;
			LoadMap3();
			eatOrb = false;
			stopsimon = false;
...
void SceneMain::CollisionState(int State)
{
	...
	for (map<int, CDistinct*>::iterator obj = Grounds->begin(); obj != Grounds->end(); obj++)//2012
	{
			...
			if (obj->second->getwhichObj() != 16)
			{
				if (IsCollision == 4 || (IsCollision == 5 && (simon->getIsFalling() == 1 && (abs(simon->gety() - 32 - (obj->second->getObjY() + obj->second->getObjH() / 2))) <= 16))){
					if (simon->gety() > obj->second->getObjY()){
						float k = obj->second->getObjY() + obj->second->getObjH() / 2 + simon->getHeight() / 2 + 5;
						simon->StopFall(k); sound->PlaySoundChoose(5);
					}
				}
		...
		for (map<int, CDistinct*>::iterator obj1 = Enemys->begin(); obj1 != Enemys->end(); obj1++){
			...
			if (obj1->second->getwhichObj() != 16)
			{
				if (IsCollision == 4)
				{
					if (obj1->second->getwhichObj() == 13)
					{
						obj1->second->setMaxMin(obj->second->getObjX() - obj->second->getObjW() / 2, obj->second->getObjX() + obj->second->getObjW() / 2);
					}
					obj1->second->StopFall(obj1->second->getObjY() - 14);
					if (obj1->second->getwhichObj() == 8){
						obj1->second->coliFish = true;
					}
					Effects.clear();
				}
		...
		for (int i = 0; i < weaponsEnemy.size(); i++)
		{
			// Con rắn medusa va chạm với nền, tường
			if (weaponsEnemy.at(i)->GetType() == 2)
			{
				int t = Collide(weaponsEnemy.at(i)->GetBox(cam), obj->second->GetBox(cam), CollisionTime, nx, ny);
				// va chạm với nền
				if ((t == 4 || t == 5) && weaponsEnemy.at(i)->IsFalling == 1)
				{
					weaponsEnemy.at(i)->y = obj->second->getObjY() + obj->second->getObjH() / 2 + 10;
					weaponsEnemy.at(i)->IsFalling = 0;
				}
				if (t == 1 || t == 2)
				{
					weaponsEnemy.at(i)->IsFalling = 2;
				}
			}
		}
	}
	...
	for (map<int, CDistinct*>::iterator obj = Enemys->begin(); obj != Enemys->end(); obj++){
		if (obj->second->getwhichObj() == 8 && obj->second->getIsGoing() == 1){
			if (obj->second->coliBrick == false){
				obj->second->Jump();
				obj->second->Fall();
				obj->second->coliFish = false;
				if (obj->second->getObjY() <= 96 && obj->second->endEffect == false){
					obj->second->endEffect = true;
					obj->second->setWater(0);
				}
			}
		}
		...
		if ((obj->second->getwhichObj() == 29||obj->second->getwhichObj()==30 || obj->second->getwhichObj()==31|| obj->second->getwhichObj() == 5 || obj->second->getwhichObj() == 6) && obj->second->getIsDie() == 0)
		{
			...
			if (IsCollision)
			{
				if ((obj->second->getwhichObj() == 29 && cam->viewport.y != 480)|| obj->second->getwhichObj() == 30||obj->second->getwhichObj() == 31){//Cục gạch ẩn có cục thịt, cục gạch ẩn
					...
					if (obj->second->getwhichObj() != 30) sound->PlaySoundChoose(9);
	...
	for (map<int, CItems*>::iterator obj = QT->getListItem()->begin(); obj != QT->getListItem()->end(); obj++)
	{
		...
		if (IsCollision) {
			...
			{
				sound->PlaySoundChoose(2); //Âm thanh trứng
				...
				if (obj->second->getwhichObj() == 17) //Quả trứng chỗ con boss dơi, đụng con đó thì tính điểm, thời gian còn lại
				{
					eatOrb = true;
					sound->StopSound();
					soundNow = -99; //Ko play nhac nen nua
					sound->PlaySoundChoose(7); //11102017
					//score->CalculatorOrb();
				}
			}
			obj->second->setIsDie(1);
