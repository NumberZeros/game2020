...
CIHolyCross::CIHolyCross(int x1, int y1, int w1, int h1)
{
	x = x1;
	y = y1;
	h = h1;
...
CIHolyCross::~CIHolyCross()
{

}
Box CIHolyCross::GetBox(GCamera *camera)
{
	D3DXVECTOR2 pos = camera->Transform(x - w / 2, y + h / 2);
	return Box(pos.x, pos.y, w, h, 0, 0);
}

void CIHolyCross::Update(int t)
{
	if (IsFalling == 1)
	{
		Vy = Vy - 0.012;
